...
#define BSLOG NSLogToFile(@"%@",[BSBacktraceLogger bs_backtraceOfCurrentThread]);
#define BSLOG_MAIN NSLogToFile(@"%@",[BSBacktraceLogger bs_backtraceOfMainThread]);
#define BSLOG_ALL NSLogToFile(@"%@",[BSBacktraceLogger bs_backtraceOfAllThread]);
...
//static inline void YHCatcher_swizzleSelector(Class theClass, SEL originalSelector, SEL swizzledSelector) {
//	Method originalMethod = class_getInstanceMethod(theClass, originalSelector);
//	Method swizzledMethod = class_getInstanceMethod(theClass, swizzledSelector);
//	method_exchangeImplementations(originalMethod, swizzledMethod);
//}

static inline BOOL YHCatcher_addMethod(Class theClass, SEL selector, Method method) {
	return class_addMethod(theClass, selector,  method_getImplementation(method),  method_getTypeEncoding(method));
}
#pragma mark - YHCatcherForward class

@interface YHCatcherForward : NSObject
@property (readwrite,nonatomic,assign) Class origClass;//origClass赋值必须在oriSelector之前
@property (readwrite,nonatomic,assign) SEL oriSelector;
@end

@implementation YHCatcherForward

- (void)setOriSelector:(SEL)oriSelector
{
	if (oriSelector != _oriSelector){
		_oriSelector = oriSelector;
	}
	Method method = class_getInstanceMethod(_origClass, _oriSelector);
	const char *returnType = method_copyReturnType(method);
	
	if (returnType == NULL) {//如果野指针类不存在该SEL方法，则直接当空返回处理
		if (![self respondsToSelector:_oriSelector]) {
			YHCatcher_addMethod([self class], _oriSelector, class_getInstanceMethod([self class], @selector(handleForwardMethodReturnVoid)));
		}
		return;
	}
	
	//目前处理两种返回类型就可以了，一种是id类型的返回nil，其他的都当做void类型处理
	switch (returnType[0] == 'r' ? returnType[1] : returnType[0]) {
		case '@':{
			if (![self respondsToSelector:_oriSelector]) {
				YHCatcher_addMethod([self class], _oriSelector, class_getInstanceMethod([self class], @selector(handleForwardMethodReturnid)));
			}
		}
			break;

		default: {
			if (![self respondsToSelector:_oriSelector]) {//默认情况，当做空返回处理
				YHCatcher_addMethod([self class], _oriSelector, class_getInstanceMethod([self class], @selector(handleForwardMethodReturnVoid)));
			}
		}
			break;
...
- (void)handleForwardMethodReturnVoid
{
...
#pragma mark - YHCatchProxy class


#define logWildPointerMsg(fmt, ...)	[AppDelegate saveLogMsg:[NSException exceptionWithName:@"wuyuan-Log" \
reason:[NSString stringWithFormat:@"[%@:%d:%s]" fmt, \
[[[NSString stringWithUTF8String:__FILE__] pathComponents] lastObject],	\
__LINE__,\
__FUNCTION__,\
##__VA_ARGS__]	\
userInfo:nil] isCrash:YES]

@interface YHCatchProxy ()
@end

@implementation YHCatchProxy


//+ (BOOL)resolveClassMethod:(SEL)sel
//{
//	NSLog(@"发现objc野指针:%s::%p=>%@",class_getName(self.currentClass),self,NSStringFromSelector(sel));
//	return nil;
//}
...
- (id)forwardingTargetForSelector:(SEL)aSelector
{
	...
	[self logMsgInHourTimeWithSelector:NSStringFromSelector(aSelector)];
	
//	NSAssert(0, nil);
	
	YHCatcherForward *yhcf = [[YHCatcherForward alloc]init];
	yhcf.origClass = self.origClass;
	yhcf.oriSelector = aSelector;
...
- (void)dealloc
{
	...
	[self logMsgInHourTimeWithSelector:@"dealloc"];
	
	NSAssert(0, nil);
}

- (void)logMsgInHourTimeWithSelector:(NSString *)Selector
{
	//防止频繁上报日志，每个小时只上报一次
//	NSString *todayStr = [EntrysOperateHelper stringFromDate:[EntrysOperateHelper serverDate] andGMT8Format:@"yyyy-MM-dd HH"];
//	NSUserDefaults *account = [NSUserDefaults standardUserDefaults];
//	NSString *currentDateStr = [account valueForKey:@"wpCurrentDate"];
//	if (![currentDateStr isEqualToString:todayStr]) {
//		[account setValue:todayStr forKey:@"wpCurrentDate"];
//		[[NSUserDefaults standardUserDefaults] synchronize];
//		
//		NSString *msg = [NSString stringWithFormat:@"%s::%p=>%@",class_getName(self.origClass),self,Selector];
////		logWildPointerMsg(@"WildPointer==>:%@",msg);
//	}
}
